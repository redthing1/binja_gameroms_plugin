...
class NDSRom:
    ...
    arm9_overlay_table: NDSOverlayTable
    arm7_overlay_table: NDSOverlayTable
    fat_entries: List[FATEntry]
    directory_entries: Dict[int, FNTDirectoryEntry]
    rom_data: bytes


class NDSRomReader:
    @staticmethod
    def read(rom_data: bytes) -> NDSRom:
        header = NDSRomReader._read_cartridge_header(rom_data)
        arm9_overlay_table = NDSRomReader._read_overlay_table(
            rom_data, header.arm9_overlay_offset, header.arm9_overlay_size
        )
        arm7_overlay_table = NDSRomReader._read_overlay_table(
            rom_data, header.arm7_overlay_offset, header.arm7_overlay_size
        )
        fat_entries = NDSRomReader._parse_fat(
            rom_data, header.fat_offset, header.fat_size
        )
        directory_entries = NDSRomReader._parse_fnt(
            rom_data, header.fnt_offset, header.fnt_size
        )
    ...
    @staticmethod
    def _read_cartridge_header(rom_data: bytes) -> NDSCartridgeHeader:
        header_data = rom_data[:0x200]  # Read the entire header (512 bytes)

        return NDSCartridgeHeader(
            ...
            arm9_auto_load_list_ram_address=struct.unpack_from("<I", header_data, 0x70)[
                0
            ],
            arm7_auto_load_list_ram_address=struct.unpack_from("<I", header_data, 0x74)[
                0
            ],
    ...
    @staticmethod
    def _read_overlay_table(rom_data: bytes, offset: int, size: int) -> NDSOverlayTable:
        entries = []
    ...
    @staticmethod
    def _parse_fat(rom_data: bytes, fat_offset: int, fat_size: int) -> List[FATEntry]:
        fat_entries = []
    ...
    def is_valid(rom_data: bytes) -> bool:
        ...
        if header_crc != calculated_header_crc:
            print("Header CRC check failed, are you sure your header/ROM is valid?")
            return False

        # Check secure area CRC
        secure_crc = struct.unpack_from("<H", header_data, 0x15C)[0]
        calculated_secure_crc = NDSRomReader._crc16(header_data[0xC0:0x15C])
        if secure_crc != calculated_secure_crc:
            print(
                "Secure area CRC check failed, are you sure your header/ROM is valid?"
            )
            return False
...
class NDSRomPrinter:
    ...
    @staticmethod
    def _print_fat(fat_entries: List[FATEntry]):
    ...
    @staticmethod
    def _print_file_system(
        directory_entries: Dict[int, FNTDirectoryEntry],
        rom_data: bytes,
        fnt_offset: int,
    ):
        print("\nFile Name Table (FNT):")
        NDSRomPrinter._print_directory(directory_entries, rom_data, fnt_offset, 0xF000)
    ...
    def _print_directory(
        ...
        sub_table = NDSRomReader._parse_fnt_sub_table(
    ...
    @staticmethod
    def _print_overlay_tables(arm9_ovt: NDSOverlayTable, arm7_ovt: NDSOverlayTable):
        NDSRomPrinter._print_overlay_table("ARM9", arm9_ovt)
        NDSRomPrinter._print_overlay_table("ARM7", arm7_ovt)

    @staticmethod
    def _print_overlay_table(name: str, ovt: NDSOverlayTable):
        print(f"\n{name} Overlay Table:")
        print("=" * (len(name) + 15))
        for i, entry in enumerate(ovt.entries):
            ...
            print(
                f"  Static Initializer End: 0x{entry.static_initializer_end_address:08X}"
            )
